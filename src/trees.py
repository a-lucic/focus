...
def _exact_activation_by_index(
        feat_input, feat_index, threshold):
  boolean_act = tf.math.greater(feat_input[:, feat_index], threshold)
  return tf.logical_not(boolean_act), boolean_act

def _approx_activation_by_index(
        feat_input, feat_index, threshold, sigma):
  activation = tf.math.sigmoid((feat_input[:, feat_index] - threshold)*sigma)
  return 1. - activation, activation

def _double_activation_by_index(
        feat_input, feat_index, threshold, sigma):
  e_l, e_r = _exact_activation_by_index(
                feat_input, feat_index, threshold)
  a_l, a_r = _approx_activation_by_index(
                feat_input, feat_index, threshold, sigma)
  return (e_l, a_l), (e_r, a_r)

def _split_node_by_index(node, feat_input, feat_index,
                         threshold, sigma):
  # exact node and approximate node
  e_o, a_o = node
  ((e_l, a_l),
   (e_r, a_r)) = _double_activation_by_index(feat_input,
                                             feat_index,
                                             threshold,
                                             sigma)
  return ((tf.logical_and(e_l,e_o), a_l*a_o),
          (tf.logical_and(e_r,e_o), a_r*a_o))

def _split_approx(node, feat_input, feat_index, threshold, sigma):
  if node is None:
    node = 1.
  l_n, r_n = _approx_activation_by_index(
                feat_input, feat_index, threshold, sigma)
...
def _split_exact(node, feat_input, feat_index, threshold):
  if node is None:
    node = True
  l_n, r_n = _exact_activation_by_index(
                feat_input, feat_index, threshold, sigma)
...
def _parse_class_tree(tree, feat_columns, feat_input, split_function):
  ...
  for i in range(n_nodes):
    ...
    if (children_left[i] != children_right[i]):
      l_n, r_n = split_function(cur_node,
                                      feat_input,
                                      feature[i],
                                      threshold[i])
...
def get_prob_classification_tree(tree, feat_columns, feat_input, sigma):
  ...
  if tree.tree_.node_count > 1:
    ...
    only_class = tree.predict(tf.reshape(feat_input[0, :], shape=(1, -1)))      # can differ depending on particular samples used to train each tree

    correct_class = tf.constant(1, shape=(len(feat_input)), dtype=tf.float64)   # prob(belong to correct class) = 100 since there's only one node
    incorrect_class = tf.constant(0, shape=(len(feat_input)), dtype=tf.float64) # prob(wrong class) = 0
    if only_class == 1.0:
...
def get_prob_classification_forest(model, feat_columns, feat_input, sigma=10., temperature=1.):
  ...
  if isinstance(model, AdaBoostClassifier) == True:
...
# def get_exact_classification_forest(model, feat_columns, feat_input):
#   tree_parser = lambda x: get_exact_classification_tree(
#                               x, feat_columns, feat_input)
...
def parse_boosted_classification_forest(model, feat_columns, feat_input, sigma=1., temperature=1.):
  ...
  for class_name in model.classes_:
    ...
    unnormalized_prob[class_name] = tf.exp(temperature*sum(w*tree[1][class_name] for w, tree in zip(weights, tree_l)))
    prob_denom += unnormalized_prob[class_name]

  max_result = tf.reduce_max(
                  tf.stack(
                    [inter_result[class_name] for class_name in model.classes_]
                  ), axis=0)
  exact_result = {}
  prob_result = {}
  for class_name in model.classes_:
